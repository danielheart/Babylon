...
// ************搭建threejs场景，导入模型**********
let scene, renderer, camera, faceObj, upperJaw, lowerJaw
...
export function plydata(data) {
   let object
   if (data.object === 'upperjaw') object = upperJaw
   else if (data.object === 'lowerjaw') object = lowerJaw
   return {
      type: 'plydata',
      object: data.object,
      color: object.geometry.attributes.color.array,
...
export function matrix(data) {
   const matrixWorld = convertBto3Matrix(data.array)
   // 将物体应用变换矩阵
   if (data.object === 'face') {
